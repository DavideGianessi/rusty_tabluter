...
fn extract_bit(bitboard: u128, idx: i8) -> bool {
    let mask = if idx < 0 || idx > 127 { 0 } else { 1 };
    ((bitboard.wrapping_shr(idx as u32)) & mask) != 0
}

#[inline(always)]
fn extract_bit_inplace(bitboard: u128, idx: i8) -> bool {
    let bitmask = 1u128.wrapping_shl(idx as u32);
    let range_mask = if idx < 0 || idx > 127 { 0 } else { !0u128 };
...
impl State {
    ...
    fn generate_piece_moves(&self, r: u8, c: u8, moves: &mut Vec<Move>) {
        ...
        for &(dr, dc) in &directions {
            ...
            while curr_r >= 0 && curr_r < 9 && curr_c >= 0 && curr_c < 9 {
                ...
                let captured = self.compute_captures(r, c, tr, tc);
                moves.push(Move {
                    fr: r,
                    fc: c,
                    tr,
    ...
    pub fn compute_full_hash(&mut self) {
        for i in 0..8 {
            ...
            while w != 0 {
                let idx = w.trailing_zeros() as u8;
                h ^= Z_WHITE[i][idx];
                w &= w - 1;
            }
            ...
            let mut k = self.king;
            while k != 0 {
                let idx = k.trailing_zeros() as u8;
                h ^= Z_KING[i][idx];
                k &= k - 1;
            }
    ...
    pub fn update_hash(&mut self, mv: &Move) {
        let fr_idx = (mv.fr * 9 + mv.fc) as u8;
        let tr_idx = (mv.tr * 9 + mv.tc) as u8;
    ...
    pub fn apply_move(&mut self, mv: &Move, history: &Vec<u64>) {
        ...
        let fr_idx: u8 = mv.fr * 9 + mv.fc;
        let to_idx: u8 = mv.tr * 9 + mv.tc;
